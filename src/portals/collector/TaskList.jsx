...
export default function TaskList() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        //   ...(search && { search }),
        //   ...(filter !== 'All' && { status: filter.toLowerCase().replace(' ', '_') }) })
        // const res = await fetch(`/api/collector/tasks?${params}`, { headers: { Authorization: `Bearer ${token}` } })
        // if (!res.ok) throw new Error('Failed to load tasks')
        // const json = await res.json()
        // setTasks(json.data); setTotal(json.total)
        throw new Error('Task list API not connected — wire up /api/collector/tasks')
      } catch (err) {
  ...
  async function updateStatus(taskId, newStatus) {
    // TODO: PATCH /api/collector/tasks/:id { status: newStatus }
    ...
    <div className="p-6 flex flex-col gap-6 max-w-6xl">
      ...
      <Card>
        {loading ? (
          ...
          <>
            <Table headers={['ID', 'Category', 'Priority', 'Location', 'Assigned At', 'Status', 'Actions']}>
              {tasks.map((t) => (
                <tr key={t.id} className="hover:bg-forest-800/20 transition-colors">
                  ...
                  <Td className="max-w-[180px] truncate text-sand-500">{t.location}</Td>
                    ...
                    <div className="flex gap-2">
                      {t.status === 'assigned' && (
                        <button
                          ...
                          className="text-xs font-mono px-2.5 py-1 rounded-lg bg-sand-500/15 text-sand-300 border border-sand-500/30 hover:bg-sand-500/25 transition-colors"
                        >
                          Start
                        </button>
                      )}
                      {t.status === 'in_progress' && (
                        <button
                          ...
                          className="text-xs font-mono px-2.5 py-1 rounded-lg bg-forest-500/15 text-forest-300 border border-forest-500/30 hover:bg-forest-500/25 transition-colors"
                        >
                          Complete
                        </button>
                      )}
                      {t.status === 'in_progress' && (
                        <button
                          ...
                          className="text-xs font-mono px-2.5 py-1 rounded-lg bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20 transition-colors"
                        >
                          Cancel
                        </button>
                      )}
